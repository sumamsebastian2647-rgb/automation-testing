...
test.describe('Trainer Management Tooltip Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test('Verify tooltip for Inactive Trainer action icon', async ({ page }) => {
        // Locate the first "view" icon
        const viewIcon = page.locator('a[title="Inactive Trainer"]').first();
        // Hover over it (optional for title tooltip)
        await viewIcon.hover();
        // Get the tooltip text
        const tooltipText = await viewIcon.getAttribute('title');
        // Assert tooltip text
        expect(tooltipText).toBe('Inactive Trainer');
        // Log success if passed
        if (tooltipText === 'Inactive Trainer') {
            console.log(`✅ Test Passed: Tooltip verified successfully and it is "${tooltipText}"`);
        } else {
            console.log(`❌ Test Failed: Expected "Inactive Trainer" but found "${tooltipText}"`);
        }
    });
    test('Verify tooltip for Login as Trainer action icon', async ({ page }) => {
        // Locate the first "view" icon
        const viewIcon = page.locator('a[title="Login as Trainer"]').first();
        // Hover over it (optional for title tooltip)
        await viewIcon.hover();
        // Get the tooltip text
        const tooltipText = await viewIcon.getAttribute('title');
        // Assert tooltip text
        expect(tooltipText).toBe('Login as Trainer');
        // Log success if passed
        if (tooltipText === 'Login as Trainer') {
            console.log(`✅ Test Passed: Tooltip verified successfully and it is "${tooltipText}"`);
        } else {
            console.log(`❌ Test Failed: Expected "Login as Trainer" but found "${tooltipText}"`);
        }
    });
