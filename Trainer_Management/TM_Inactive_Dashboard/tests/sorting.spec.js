...
test.describe('Admin Table Sorting Tests', () => {
  ...
  //////////////////////////////////////////////////////
test('Verify Name column sorting behavior', async ({ page }) => {
  const normalize = arr => arr.map(v => v.trim());

  // -------------------- Locators --------------------
  const nameHeaderSelector = 'table thead tr th[data-col-seq="1"] a';
  const nameHeader = page.locator(nameHeaderSelector);
  ...
  console.log('Initial name order:', initialOrder);
  
  // Verify we have data before proceeding
  expect(initialOrder.length).toBeGreaterThan(0);
  
  // -------------------- First Sort (A-Z) --------------------
  console.log('Clicking Name header for first sort...');
  ...
  // -------------------- Second Sort (Z-A) --------------------
  console.log('Clicking Name header again for second sort...');
  await nameHeader.click();
  await page.waitForLoadState('networkidle', { timeout: 10000 });
  await page.waitForTimeout(1000); // Extra buffer for sort to complete
  ...
  console.log('Clicking Name header a third time...');
  await nameHeader.click();
  await page.waitForLoadState('networkidle', { timeout: 10000 });
  await page.waitForTimeout(1000); // Extra buffer for sort to complete
  ...
  // Check if first sort is actually alphabetical (A-Z)
  const isFirstSortAZ = [...savedFirstSortOrder].sort((a, b) => 
    a.toLowerCase().localeCompare(b.toLowerCase())
  ).join(',') === savedFirstSortOrder.join(',');
  
  // Check if second sort is reverse alphabetical (Z-A)
  const isSecondSortZA = [...savedFirstSortOrder].sort((a, b) => 
    b.toLowerCase().localeCompare(a.toLowerCase())
  ).join(',') === secondSortOrder.join(',');
  
  console.log(`First sort appears to be A-Z: ${isFirstSortAZ}`);
  console.log(`Second sort appears to be Z-A: ${isSecondSortZA}`);
  
  // Optional: Verify sort pattern makes sense (first element of each sort)
  console.log(`First name in A-Z sort: ${firstSortOrder[0]}`);
  console.log(`First name in Z-A sort: ${secondSortOrder[0]}`);
  
  console.log('Name sorting test passed ✅');
});
