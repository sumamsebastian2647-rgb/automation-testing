...
const createformConfig = require('./createform_config');
const customFieldConfig = createformConfig.customFieldConfig;
...
async function fillResidentialDetails(page) {
 ...
 await fillAllCustomFieldsInStep(page, 'Step2');
  await page.getByRole('button', { name: 'Next' }).click();
}
async function fillAllCustomFieldsInStep(page, stepId) {
  const step = page.locator(`#${stepId}`);
  await step.waitFor({ state: 'visible', timeout: 15000 });
  const wrappers = step.locator('.custom-field-wrapper');
  const count = await wrappers.count();
  console.log(`Found ${count} custom fields in ${stepId}`);
  for (let i = 0; i < count; i++) {
    const wrapper = wrappers.nth(i);
    const type = await wrapper.getAttribute('data-field-type');
    console.log(`Filling field ${i + 1}/${count} → ${type}`);
    switch (type) {
      case 'text':
        await wrapper.locator('input').fill(customFieldConfig.text);
        break;
      case 'email':
        await wrapper.locator('input').fill(customFieldConfig.email);
        break;
      case 'number':
        await wrapper.locator('input').fill(customFieldConfig.number);
        break;
      case 'textarea':
        await wrapper.locator('textarea').fill(customFieldConfig.textarea);
        break;
      case 'date':
        await wrapper.locator('input').fill(customFieldConfig.date);
        break;
      case 'dropdown':
        await wrapper.locator('select').selectOption({ index: customFieldConfig.dropdown.value });
        break;
      case 'radio':
        await wrapper
          .locator('input[type="radio"]')
          .nth(customFieldConfig.radio.index)
          .check({ force: true });
        break;
      case 'checkbox':
        await wrapper
          .locator('input[type="checkbox"]')
          .nth(customFieldConfig.checkbox.index)
          .check({ force: true });
        break;
      case 'file':
        await wrapper
          .locator('input[type="file"]')
          .setInputFiles(customFieldConfig.file);
        break;
      default:
...
async function fillContactInformation(page) {
  ...
  await fillAllCustomFieldsInStep(page, 'Step3');
  await page.getByRole('button', { name: 'Next' }).click();
}
async function fillEmployerDetails(page) {
  ...
  await fillAllCustomFieldsInStep(page, 'Step4');
  await page.getByRole('button', { name: 'Next' }).click();
}
async function fillPreCourseEvaluation(page) {
  ...
  await page.waitForTimeout(1000); // Wait for the page to settle
  // Use JavaScript to trigger the radio button click since the UI elements are hidden
  try {
    // Try to find the actual radio input and trigger it with JavaScript
    await page.evaluate(() => {
      const radioInputs = document.querySelectorAll('.block input[type="radio"]');
    ...
    // Fallback: try to trigger the iCheck element with JavaScript
    await page.evaluate(() => {
      const iCheckElement = document.querySelector('.block > .iradio_flat-red');
      if (iCheckElement) {
        iCheckElement.click();
      }
    });
  ...
  await page.waitForTimeout(500); // Wait for the element to be ready
  // Use JavaScript to trigger the radio button click since the UI elements are hidden
  try {
    // Try to find the actual radio input and trigger it with JavaScript
    await page.evaluate(() => {
      const radioInputs = document.querySelectorAll('.txtPriorEducationFlag input[type="radio"]');
      if (radioInputs.length > 0) {
        radioInputs[0].click();
        return true;
      }
    ...
    // Fallback: try to trigger the iCheck element with JavaScript
    await page.evaluate(() => {
      const iCheckElement = document.querySelector('.txtPriorEducationFlag > .iradio_flat-red');
      if (iCheckElement) {
        iCheckElement.click();
      }
    });
  ...
  await fillAllCustomFieldsInStep(page, 'Step5');
  await page.getByRole('button', { name: 'Next' }).click();
}
async function rpl(page) {
   ...
   await fillAllCustomFieldsInStep(page, 'Step6');
   await page.getByRole('button', { name: 'Next' }).click();
...
async function fillStudentIdentifiers(page) {
  ...
  await fillAllCustomFieldsInStep(page, 'Step7');
  await page.getByRole('button', { name: 'Next' }).click();
...
async function uploadDocuments(page) {
  ...
  await fillAllCustomFieldsInStep(page, 'Step8');
  await page.getByRole('button', { name: 'Next' }).click();
