...
test.describe('@course @clone Clone Course Module', () => {
  ...
  /*test('@regression remove copy from cloned code and name and it should prevent duplicate save', async ({ page }) => {
    const { dashboardPage } = await openCloneCourseForm(page);
    ...
    await expect(page.locator('.help-block-error'))
      .toContainText(/same code.*exists/i, { timeout: 10000 });
  });*/
