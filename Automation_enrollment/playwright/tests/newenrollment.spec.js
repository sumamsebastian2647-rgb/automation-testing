...
const { expandFormsMenu, generateEnrollmentLink } = require('../generatelink.js');
...
class EnrollmentChecker {
  ...
  async checkAlreadyEnrolled() {
    try {
      // Wait briefly for popup to appear if it's going to
      ...
      if (isVisible) {
        const hasMessage = await this.page.locator('#swal2-content')
